...
function Shell({ user, onLogout }: { user: AuthUser, onLogout: () => void }) {
  ...
  return <div className="app-shell"><aside className={`sidebar ${mobile ? 'open' : ''}`}><button className="close-mobile" onClick={() => setMobile(false)}><X /></button><div className="brand"><div className="logo-orbit"><img src="/govt-sindh-logo.jpg" /></div><div><strong>HRMIS</strong><span>Health Department<br />Government of Sindh</span></div></div><nav><button className={page === 'profile' ? 'active' : ''} onClick={() => { setSelectedEmployeeId(null); setPage('profile'); setMobile(false) }}><UserRound />User Profile</button><button className={(page === 'leaves' || page === 'employee') ? 'active' : ''} onClick={() => { setSelectedEmployeeId(null); setPage('leaves'); setMobile(false) }}><CalendarDays />Leave Requests</button><button disabled><ArrowRight />Transfer Requests <em>Soon</em></button><button disabled><Activity />Disciplinary Actions <em>Soon</em></button></nav><div className="powered"><img src="/hrmis-powered-by.png" /></div></aside><main><header><button className="menu-btn" onClick={() => setMobile(true)}><Menu /></button><div><small>Welcome Back Dr. {displayName}!</small><h1>{pageTitle}</h1></div><div className="header-actions"><div className="notification-wrap" ref={notificationRef}><button className="bell-btn" aria-label="Notifications" onClick={() => setBellOpen(v => !v)}><Bell />{unread > 0 && <i>{unread > 99 ? '99+' : unread}</i>}</button>{bellOpen && <div className="notification-panel"><div className="notification-head"><b>Notifications</b><button onClick={markAll}>Mark all as read</button></div><div className="notification-list">{notifications.length === 0 ? <p className="empty-notifications">No notifications yet.</p> : notifications.map(n => <button key={n._id} className={n.read ? '' : 'unread'} onClick={() => openNotification(n)}><span className="notification-icon"><FileText /></span><span className="notification-copy"><b>{n.title}</b><p>{n.message}</p><small>{new Date(n.createdAt).toLocaleString()}</small></span>{!n.read && <i className="notification-dot" />}</button>)}</div><button className="notification-view-all" onClick={() => { setBellOpen(false); setPage('notifications'); }}>View all notifications</button></div>}</div><div className="avatar">{initials}</div><div className="header-user"><span>{user.personnelNo}</span><b>{user.email}</b></div><button className="header-logout" onClick={onLogout} title="Sign out" aria-label="Sign out"><LogOut /></button></div></header><motion.div key={page} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .25 }} className="page-wrap">{page === 'profile' ? <ProfilePage /> : page === 'leaves' ? <LeavePage role={user.role} onOpenEmployee={openEmployee} /> : page === 'notifications' ? <NotificationsPage onOpenLeave={(n) => openNotification(n)} /> : selectedEmployeeId ? <EmployeeOverviewPage userId={selectedEmployeeId} role={user.role} onBack={() => { setSelectedEmployeeId(null); setPage('leaves'); }} /> : <LeavePage role={user.role} onOpenEmployee={openEmployee} />}</motion.div></main></div>
}
...
function EmployeeForm({ profile }: { profile: any }) {
  return <div className="readonly-profile"><div className="section-head"><div><h3>Employee Information</h3></div><UserRound /></div><div className="profile-readonly-note">Profile information is read-only for now.</div><div className="grid2">
...
function NewLeave({ onSubmitted }: { onSubmitted: () => void }) {
  ...
  return <div className="leave-layout">
    <form className="content-card" onSubmit={submit}>
      <div className="section-head"><div><h3>Apply for leave</h3></div><Sparkles /></div>
...
function LeaveHistory({ mode, role, refreshKey, onChanged, onOpenEmployee }: { mode: 'mine' | InternalView; role: Role; refreshKey: number; onChanged?: () => void; onOpenEmployee?: (userId: string) => void }) {
  ...
  const title = mode === 'mine' ? 'Leave History' : mode === 'pending' ? 'Pending Employee Requests' : 'Processed Leave History';
